      ...
      <style>{`
        ...
        *,*::before,*::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        .bg-pat  { position:fixed; inset:0; opacity:var(--msj-pattern-opacity); z-index:0;
          background-image:url("data:image/svg+xml,%3Csvg width='120' height='120' viewBox='0 0 120 120' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' stroke='%23c9a84c' stroke-width='0.6'%3E%3Cpolygon points='60,6 114,33 114,87 60,114 6,87 6,33'/%3E%3C/g%3E%3C/svg%3E");
          background-size:120px 120px;
        }
        .bar   { position:fixed; left:0; right:0; height:2px; z-index:10; background:var(--msj-bar); }
        .bar-t { top:0; } .bar-b { bottom:0; }

        .hub-root { min-height:100vh; position:relative; overflow-x:hidden; padding-bottom:80px; }
        .page     { position:relative; z-index:1; max-width:430px; margin:0 auto; }
        ...
        .tab-btn.active  { background:var(--msj-badge-bg); border-color:var(--msj-gold-border); color:var(--msj-text-title); }
        .tab-btn.inactive { background:var(--msj-card-bg); border-color:var(--msj-card-border); color:var(--msj-text-sub); }
        .tab-count { font-size:10px; padding:1px 6px; border-radius:10px; background:var(--msj-card-bg); }
        .tab-btn.active .tab-count { background:var(--msj-gold-bg); color:var(--msj-gold-bright); }

        /* Section label */
        .sec-label  { display:flex; align-items:center; gap:8px; padding:0 20px; margin-bottom:10px; }
        .sec-line   { flex:1; height:1px; }
        .sl-left    { background:var(--msj-divider-line-l); }
        .sl-right   { background:var(--msj-divider-line-r); }
        .sec-text   { font-size:9px; color:var(--msj-divider-text); letter-spacing:2.5px; text-transform:uppercase; white-space:nowrap; }

        .cards { display:flex; flex-direction:column; gap:10px; padding:0 20px; margin-bottom:20px; }

        /* Pengumuman */
        .p-card       { background:var(--msj-card-bg); border:1px solid var(--msj-card-border); border-radius:16px; overflow:hidden; cursor:pointer; transition:all .2s; }
        .p-card:hover { border-color:var(--msj-gold-border); }
        .p-card.open  { border-color:var(--msj-gold-border); }
        .p-card::before { content:''; display:block; height:1px; background:linear-gradient(90deg,transparent,var(--msj-gold-border),transparent); }
        .p-top    { padding:14px 16px; display:flex; align-items:flex-start; gap:10px; }
        .p-dot    { width:7px; height:7px; border-radius:50%; background:var(--msj-gold-bright); box-shadow:0 0 6px rgba(201,168,76,.5); flex-shrink:0; margin-top:5px; }
        .p-main   { flex:1; min-width:0; }
        .p-judul  { font-size:13.5px; font-weight:600; color:var(--msj-text-title); margin-bottom:3px; line-height:1.3; }
        .p-tgl    { font-size:10px; color:var(--msj-gold-text); }
        .p-chev   { opacity:.3; transition:transform .2s; flex-shrink:0; }
        .p-card.open .p-chev { transform:rotate(180deg); }
        .p-body       { padding:0 16px 14px 33px; font-size:12.5px; color:var(--msj-text-desc); line-height:1.7; border-top:1px solid var(--msj-card-border); }
        .p-body-inner { padding-top:10px; }

        /* Kajian */
        .k-card       { background:var(--msj-card-bg); border:1px solid rgba(52,152,219,.12); border-radius:16px; padding:14px 16px; cursor:pointer; transition:all .2s; overflow:hidden; }
        ...
        .k-day    { font-size:18px; font-weight:700; color:var(--msj-text-title); line-height:1; }
        ...
        .k-judul  { font-size:13.5px; font-weight:600; color:var(--msj-text-title); margin-bottom:4px; line-height:1.3; }
        .k-meta   { font-size:11px; color:var(--msj-text-sub); display:flex; flex-direction:column; gap:2px; }
        .k-body   { margin-top:10px; padding-top:10px; border-top:1px solid var(--msj-card-border); font-size:12px; color:var(--msj-text-desc); line-height:1.6; }
        ...
        /* Infaq saldo */
        .saldo-card  { background:var(--msj-green-bg); border:1px solid var(--msj-green-border); border-radius:16px; padding:16px; margin-bottom:10px; }
        .saldo-top   { text-align:center; margin-bottom:12px; }
        .saldo-label { font-size:9.5px; font-weight:600; letter-spacing:2px; text-transform:uppercase; color:var(--msj-green-text); margin-bottom:4px; opacity:.7; }
        .saldo-value { font-family:'Playfair Display',serif; font-size:24px; font-weight:700; color:var(--msj-text-title); }
        .saldo-row   { display:grid; grid-template-columns:1fr 1fr; gap:8px; }
        .saldo-mini  { background:var(--msj-card-bg); border:1px solid var(--msj-card-border); border-radius:10px; padding:10px; text-align:center; }
        .sm-label    { font-size:9px; font-weight:600; text-transform:uppercase; letter-spacing:1px; margin-bottom:4px; }
        .sm-masuk    { color:rgba(46,204,113,.6); }
        .sm-keluar   { color:rgba(231,76,60,.6); }
        .sm-value    { font-size:13px; font-weight:700; }

        /* Infaq list */
        .i-card    { background:var(--msj-card-bg); border:1px solid var(--msj-card-border); border-radius:12px; padding:12px 14px; display:flex; align-items:center; gap:10px; }
        ...
        .i-ket     { font-size:12.5px; font-weight:600; color:var(--msj-text-title); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
        .i-tgl     { font-size:10px; color:var(--msj-text-muted); margin-top:1px; }
        .i-jml     { font-size:13px; font-weight:700; white-space:nowrap; }
        .jm { color:#2ecc71; } .jk { color:#e74c3c; }

        /* Pembangunan */
        .pb-card        { background:var(--msj-card-bg); border:1px solid rgba(155,89,182,.12); border-radius:16px; padding:16px; }
        .pb-top         { display:flex; align-items:flex-start; justify-content:space-between; gap:10px; margin-bottom:10px; }
        .pb-nama        { font-size:14px; font-weight:700; color:var(--msj-text-title); line-height:1.3; }
        .pb-status      { font-size:9px; font-weight:700; padding:3px 9px; border-radius:20px; white-space:nowrap; flex-shrink:0; }
        .pb-desc        { font-size:12px; color:var(--msj-text-desc); margin-bottom:12px; line-height:1.5; }
        .pb-prog-head   { display:flex; justify-content:space-between; margin-bottom:6px; }
        .pb-prog-label  { font-size:10px; color:var(--msj-text-muted); font-weight:600; }
        .pb-prog-pct    { font-size:14px; font-weight:700; color:#9b59b6; }
        .pb-prog-bg     { height:6px; background:var(--msj-card-border); border-radius:6px; overflow:hidden; margin-bottom:12px; }
        ...
        .pb-dana-item   { background:var(--msj-card-bg); border:1px solid var(--msj-card-border); border-radius:10px; padding:10px; text-align:center; }
        .pb-dana-label  { font-size:8.5px; font-weight:600; text-transform:uppercase; letter-spacing:1px; color:var(--msj-text-muted); margin-bottom:4px; }
        .pb-dana-value  { font-size:11.5px; font-weight:700; color:var(--msj-text-title); }

        .empty      { text-align:center; padding:48px 20px; color:var(--msj-text-muted); }
        .empty-icon { font-size:36px; opacity:.3; margin-bottom:12px; }
        .loading    { text-align:center; padding:64px 20px; color:var(--msj-gold-text); }
        .spinner    { width:22px; height:22px; border:2px solid var(--msj-gold-bg); border-top-color:var(--msj-gold-bright); border-radius:50%; animation:spin .7s linear infinite; margin:0 auto 12px; }
      `}</style>
          ...
          {loading ? (
            ...
            <>
              {showPengumuman && pengumuman.length > 0 && (
                <>
                  <div className="sec-label">
                    <div className="sec-line sl-left"/><span className="sec-text">✦ Pengumuman ✦</span><div className="sec-line sl-right"/>
                  </div>
                  <div className="cards">
                    {pengumuman.map(item => (
                      <div key={`p-${item.id}`} className={`p-card ${expanded === `p-${item.id}` ? "open" : ""}`}
                        ...
                        <div className="p-top">
                          ...
                          <svg className="p-chev" width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M4 6l4 4 4-4"/></svg>
                        </div>
                        {expanded === `p-${item.id}` && (
              ...
              {showKajian && kajian.length > 0 && (
                <>
                  <div className="sec-label">
                    <div className="sec-line sl-left"/><span className="sec-text">✦ Jadwal Kajian ✦</span><div className="sec-line sl-right"/>
                  </div>
                  <div className="cards">
                    {kajian.map(item => {
              ...
              {showInfaq && infaq.length > 0 && (
                <>
                  <div className="sec-label">
                    <div className="sec-line sl-left"/><span className="sec-text">✦ Saldo Infaq ✦</span><div className="sec-line sl-right"/>
                  </div>
                  <div className="cards">
                    ...
                    {activeTab === "semua" && infaq.length > 5 && (
                      <button onClick={() => setActiveTab("infaq")} style={{background:"var(--msj-card-bg)",border:"1px solid var(--msj-card-border)",borderRadius:10,padding:"10px",color:"var(--msj-text-sub)",fontSize:12,cursor:"pointer",width:"100%"}}>
              ...
              {showPembangunan && pembangunan.length > 0 && (
                <>
                  <div className="sec-label">
                    <div className="sec-line sl-left"/><span className="sec-text">✦ Info Pembangunan ✦</span><div className="sec-line sl-right"/>
                  </div>
                  <div className="cards">
