...
export default function RootPage() {
  ...
  useEffect(() => {
    // Kalau sudah login, langsung ke home jemaah
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .root-page {
          min-height: 100vh; background: #060f09;
          display: flex; align-items: center; justify-content: center;
          font-family: 'Plus Jakarta Sans', sans-serif;
          position: relative; overflow: hidden; padding: 24px;
        }
        .bg-base { position: fixed; inset: 0; background: radial-gradient(ellipse 90% 60% at 50% 0%, #0d2e18 0%, #060f09 60%); }
        .bg-pattern {
        ...
        .glow { position: fixed; width: 700px; height: 400px; border-radius: 50%; background: radial-gradient(ellipse, rgba(16,72,38,0.45) 0%, transparent 70%); top: -80px; left: 50%; transform: translateX(-50%); pointer-events: none; }
        ...
        .wrap { position: relative; z-index: 1; width: 100%; max-width: 400px; animation: cardIn 0.7s cubic-bezier(0.22,1,0.36,1) both; }
        ...
        .bismillah { font-family: 'Amiri', serif; font-size: 17px; color: rgba(201,168,76,0.7); margin-bottom: 18px; }
        .dome-svg { display: block; margin: 0 auto 14px; filter: drop-shadow(0 0 18px rgba(201,168,76,0.3)); animation: float 4s ease-in-out infinite; }
        .masjid-title { font-family: 'Playfair Display', serif; font-size: 22px; font-weight: 700; color: #e8d8a0; margin-bottom: 4px; }
        .masjid-sub { font-size: 12px; color: rgba(255,255,255,0.3); letter-spacing: 0.5px; margin-bottom: 10px; }
        .rp-badge { display: inline-flex; align-items: center; gap: 5px; padding: 4px 14px; border: 1px solid rgba(201,168,76,0.2); border-radius: 20px; font-size: 10px; font-weight: 600; letter-spacing: 2px; text-transform: uppercase; color: rgba(201,168,76,0.5); }
        ...
        .rp-dl { background: linear-gradient(90deg, transparent, rgba(201,168,76,0.25)); }
        .rp-dr { background: linear-gradient(90deg, rgba(201,168,76,0.25), transparent); }
        .rp-dtext { font-size: 9px; color: rgba(201,168,76,0.4); letter-spacing: 2.5px; text-transform: uppercase; white-space: nowrap; }
        ...
        .role-card {
          ...
          position: relative; overflow: hidden; border: 1px solid rgba(201,168,76,0.12);
        }
        .role-card::before { content: ''; position: absolute; inset: 0; opacity: 0; transition: opacity 0.25s; }
        .role-card::after {
        ...
        .role-admin::before  { background: linear-gradient(135deg, rgba(201,168,76,0.08), transparent); }
        .role-admin::after   { background: linear-gradient(180deg, #c9a84c, #f0d080); }
        .role-admin:hover    { border-color: rgba(201,168,76,0.35); transform: translateX(4px); box-shadow: 0 8px 32px rgba(0,0,0,0.4), -4px 0 0 rgba(201,168,76,0.3); }
        .role-jemaah::before { background: linear-gradient(135deg, rgba(46,204,113,0.08), transparent); }
        .role-jemaah::after  { background: linear-gradient(180deg, #2ecc71, #27ae60); }
        .role-jemaah:hover   { border-color: rgba(46,204,113,0.35); transform: translateX(4px); box-shadow: 0 8px 32px rgba(0,0,0,0.4), -4px 0 0 rgba(46,204,113,0.3); }

        .role-icon { width: 52px; height: 52px; border-radius: 15px; flex-shrink: 0; display: flex; align-items: center; justify-content: center; }
        .ri-admin  { background: linear-gradient(135deg, rgba(201,168,76,0.15), rgba(201,168,76,0.05)); border: 1px solid rgba(201,168,76,0.22); }
        .ri-jemaah { background: linear-gradient(135deg, rgba(46,204,113,0.15), rgba(46,204,113,0.05)); border: 1px solid rgba(46,204,113,0.22); }

        .role-text { flex: 1; }
        .role-title { font-size: 15px; font-weight: 700; margin-bottom: 4px; }
        .rt-admin  { color: #e8d8a0; }
        .rt-jemaah { color: #a8f0c0; }
        .role-desc { font-size: 11.5px; color: rgba(255,255,255,0.3); line-height: 1.4; }
        ...
        .rp-footer { text-align: center; margin-top: 24px; font-family: 'Amiri', serif; font-size: 13px; color: rgba(201,168,76,0.18); animation: fadeUp 0.7s 0.28s cubic-bezier(0.22,1,0.36,1) both; }
      `}</style>
        ...
        <div className="wrap">
          ...
          <div className="rp-header">
            ...
            <svg className="dome-svg" width="110" height="80" viewBox="0 0 130 90" fill="none">
              <rect x="3" y="42" width="14" height="38" rx="2" fill="#1a5c38" stroke="#c9a84c" strokeWidth="0.7" opacity="0.8"/>
              ...
              <rect x="113" y="42" width="14" height="38" rx="2" fill="#1a5c38" stroke="#c9a84c" strokeWidth="0.7" opacity="0.8"/>
              ...
              <path d="M22 60 Q22 18 65 14 Q108 18 108 60Z" fill="#1a5c38" stroke="#c9a84c" strokeWidth="1"/>
              <path d="M30 60 Q30 26 65 22 Q100 26 100 60Z" fill="rgba(201,168,76,0.06)" stroke="rgba(201,168,76,0.2)" strokeWidth="0.7"/>
              <path d="M22 60 Q22 48 32 46 Q42 48 42 60Z" fill="#1a5c38" stroke="#c9a84c" strokeWidth="0.7" opacity="0.8"/>
              <path d="M88 60 Q88 48 98 46 Q108 48 108 60Z" fill="#1a5c38" stroke="#c9a84c" strokeWidth="0.7" opacity="0.8"/>
              <circle cx="65" cy="13" r="6" fill="none" stroke="#c9a84c" strokeWidth="1.3"/>
              <circle cx="67.5" cy="11.5" r="4.5" fill="#060f09"/>
          ...
          <div className="roles">
            ...
            <div className="role-card role-admin" onClick={() => router.push("/admin-login")}>
              ...
              <div className="role-text">
                <div className="role-title rt-admin">Masuk sebagai Admin</div>
